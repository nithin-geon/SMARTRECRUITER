...
function Tickets() {
    ...
    this.init = function () {
        try {
            ...
            this.FillPositionRequestDropdown();

            var MyID_Tickets = this.getParameterByName('ID_Resumes', '');
    ...
    this.Clear = function () {
        ...
        $('#txtMarks1').val("");
        $("#txtMarks1").removeClass("validateerror");
        $('#txtMarks2').val("");
        $("#txtMarks2").removeClass("validateerror");
        $('#txtMarks3').val("");
        $("#txtMarks3").removeClass("validateerror");
        $('#ddlPositionRequest').val(0);
        $("#ddlPositionRequest").removeClass("validateerror");
    ...
    this.ClearDescription = function () {
        ...
        $("#txtMarks3").removeClass("validateerror");
        $('#txtMarks4').val("");
        $("#txtMarks4").removeClass("validateerror");
                    ...
                    if (Mydata.length > 0) {
                        ...
                        try {
                            $.each(Mydata, function (i, value) {
                                optionhtml += '<option  StatusValidate=' + value.StatusCode + ' value=' + value.ID_Status + '>' + value.StatusName + '</option>';
                            });
                        }
                        catch (exx) {
                            alert(exx);
    ...
    this.FillPositionRequestDropdown = function () {
        try {
            $.ajax({

                url: api_url + '/Resumes/PositionRequestDropDownFill',
                cache: false,
                type: 'POST',
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
                success: function (data) {
                    ...
                    if (Mydata.length > 0) {
                        ...
                        try {
                            $.each(Mydata, function (i, value) {
                                optionhtml += '<option  value=' + value.ID_PositionRequest + '>' + value.PositionRequestName + '</option>';
                            });
                        }
                        catch (exx) {
                            alert(exx);
                    ...
                    $('#ddlPositionRequest').html(optionhtml);
    ...
    this.Validate = function () {
        ...
        if (isValidation == 1) {
            ...
            if ($('#ddlPositionRequest option:selected').val() == '0') {
                $('#ddlPositionRequest').addClass("validateerror");
                _Error++;
            }
            else {
                $('#ddlPositionRequest').removeClass("validateerror");
    ...
    this.ValidateDescription = function () {
        if (isValidation == 1) {
            ...
            var value = $("#ddlStatus1").find('option:selected').attr("statusvalidate");
            if (typeof value === "undefined") { value = 0; }
            if (value == 1) {

                if ($('#txtMarks1').val() == '' || parseInt($('#txtMarks1').val() == '' ? 0 : $('#txtMarks1').val()) > parseInt(5)) {
                    $('#txtMarks1').addClass("validateerror");
                    _Error++;
                }
                else {
                    $('#txtMarks1').removeClass("validateerror");
                }
                if ($('#txtMarks2').val() == '' || parseInt($('#txtMarks2').val() == '' ? 0 : $('#txtMarks2').val()) > parseInt(5)) {
                    $('#txtMarks2').addClass("validateerror");
                    _Error++;
                }
                else {
                    $('#txtMarks2').removeClass("validateerror");
                }
                if ($('#txtMarks3').val() == '' || parseInt($('#txtMarks3').val() == '' ? 0 : $('#txtMarks3').val()) > parseInt(5)) {
                    $('#txtMarks3').addClass("validateerror");
                    _Error++;
                }
                else {
                    $('#txtMarks3').removeClass("validateerror");
    ...
    this.Save = function () {
        try {
            var ObjBlTickets = {
                ...
                FK_PositionRequest: $('#ddlPositionRequest option:selected').val(),
    ...
    this.SaveDescription = function () {
        try {
            var TotalMarks = "";
            if ($('#txtMarks1').val() != "" || $('#txtMarks2').val() != "" || $('#txtMarks3').val() != ""  ) {
                var TotalMarkshead = "<p>&nbsp;</p><p> <span style='text-decoration: underline;'><strong>Assesment Form</strong></span></p >";

                var Marks1 = $('#txtMarks1').val() != "" ? "<p><strong>Communication Skills &nbsp;:&nbsp;&nbsp; <span style='color: #ff0000; '>" + $('#txtMarks1').val() + "&nbsp; <span style='color: #000000;'>/5</span></span></strong></p>" : "" ;
                var Marks2 = $('#txtMarks2').val() != "" ? "<p><strong>Subject Knowledge &nbsp;: &nbsp;&nbsp;<span style='color: #ff0000; '>" + $('#txtMarks2').val() + "&nbsp; <span style='color: #000000;'>/5</span></span></strong></p>" : "";
                var Marks3 = $('#txtMarks3').val() != "" ? "<p><strong>Attitude &nbsp;: &nbsp;&nbsp; <span style='color: #ff0000; '>" + $('#txtMarks3').val() + "&nbsp; <span style='color: #000000;'>/5</span></span></strong></p>" :"";
                var Marks4 = $('#txtMarks4').val() != "" ? "<p><strong>Written Test Score &nbsp;: &nbsp;&nbsp; <span style='color: #ff0000; '>" + $('#txtMarks4').val() + "&nbsp; <span style='color: #000000;'></span></span></strong></p>" : "";

                TotalMarks = TotalMarkshead + Marks4+ Marks1 + Marks2 + Marks3;
            }

                     var ObjBlTickets = {
                MasterID: $('#hfTicketsId').val() == '' ? '0' : $('#hfTicketsId').val(),
                Description: $('#txtUserDescription').val(),
                AgentNotes: $('#txtAgentNotes').val() + TotalMarks,
    ...
    this.SaveAssign = function () {
        try {
            objTickets.JavascriptFunction(1);
                        ...
                        if (Mydata.length > 0) {
                            ...
                            $.each(Mydata, function (key, val) {
                                ...
                                var htmlActionTd = "<td>" +
                                    ...
                                    "</td>" +
                                    "<td>" +
                                    "<a href ='javascript:void(0)' onclick = 'objTickets.FillTickets(" + val.ID_Tickets + ")' class='edit'><i class='glyphicon glyphicon-edit'></i>" + "</a>  &nbsp" +
                                    "<a href ='javascript:void(0)' onclick = 'objTickets.DeleteBranch(" + val.ID_Tickets + ")' class='remove'><i class='glyphicon glyphicon-trash'></i>" + "</a>" +
                                    "</td>";
                                html += "<tr>" +
                                ...
                                html += "</td><td>" + ToJavaScriptDate(val.TickDate) + "</td>" +
                                    ...
                                    "<td>" + val.UsMob + "</td>" +
                                    "<td>" + val.Usemail + "</td>" +
                                    "<td>" + val.TickStatusName + "</td>" +
                                    "<td>" + val.LastAssignAgentName;
                            ...
                            {
                                var temppagecount = parseInt(RecordCount) / 50
                                ...
                                if (parseInt(pageCount) > 0) {
                                    html += "</table><div class='box-footer clearfix'><ul class='pagination pagination-sm no-margin pull-right'>" +
                                        ...
                                        "<li><a href ='javascript:void(0)'  onclick = 'objTickets.FillGrid(" + (parseInt(i) - parseInt(50)) + ")'   class= '" + (parseInt(i) > parseInt(1) ? "activeAnchor" : "not-activeAnchor") + " '>«</a></li>"
                                    while (parseInt(i) <= parseInt(pageCount)) {
                                        ...
                                        if (parseInt(i) % parseInt(50) == 0) {
                                            break;
                                        }
                                        i++;
                                    }
                                    html += "<li><a href ='javascript:void(0)' onclick = 'objTickets.FillGrid(" + (parseInt(i) + parseInt(1)) + ")'   class= '" + ((parseInt(pageCount) - parseInt(PageIndex)) > 50 ? "activeAnchor" : "not-activeAnchor") + " ' >»</a></li>" +
    ...
    this.FillTickets = function (ID_Tickets) {
        try {
        $.ajax({
            url: api_url + "/Resumes/FillResumes",
                ...
                try {
                    ...
                    if (Mydata.length > 0) {
                        $.each(Mydata, function (key, value) {
                            ...
                            $('#ddlClient').val(objTickets.FK_Client);
                            $('#ddlStatus').val(objTickets.TickStatus);
                            $('#ddlProduct').val(objTickets.FK_Product);
                            $('#ddlDepartmentAdd').val(objTickets.FK_Department);
                            $('#ddlTopic').val(objTickets.FK_Topic);
                            $('#txtUserName').val(objTickets.candidatename);
                            $('#txtUserMobile').val(objTickets.candidatemob);
                            $('#txtUserEmailId').val(objTickets.candidateemailid);
                            $('#ddlPositionRequest').val(objTickets.FK_PositionRequest);
    ...
    this.DeleteBranch = function (ID_Tickets) {
        if (confirm('Do you want to delete..?')) {
            $.ajax({
                url: api_url + "/Resumes/DeleteResumes",
                data: { 'ID_Tickets': ID_Tickets },
                cache: false,
                type: 'Get',
                contentType: 'application/json; charset=utf-8',
