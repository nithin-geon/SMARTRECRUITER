...
namespace SupportTicket.Controllers
{
    public class PartialController : Controller
    {
        ...
        public JsonResult FillMenu()
        {
            ...
            if (dtblPages.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtblPages.Rows.Count; i++)
                {
                    ...
                    if (Convert.ToInt32(dtblNotification.Rows[0]["Tasks"]) > 0)
                    {
                        ...
                        if (dtblPages.Rows[i][3].ToString() == "Resumes")
                        {
            ...
            strNotiHtml = strNotiHtml + "<a href='#' class='dropdown-toggle' data-toggle='dropdown'><i  id ='Notificationbell' class='fa fa-bell-o swing animated'></i>";
            if (Convert.ToInt32(dtblNotification.Rows[0]["Tasks"]) > 0)
            {
                strNotiHtml = strNotiHtml + "<span id='NotificationbellCount' class='label label-danger'>" + Convert.ToInt32(dtblNotification.Rows[0]["Tasks"]) + "</span>";
            }
            strNotiHtml = strNotiHtml + "</a>";
            ...
            if (dtblNotification.Rows.Count > 0)
            {
                for (int j = 1; j < 5; j++)
                {
                    if (dtblNotification.Columns[j].ColumnName == "PendingResumes")
                    {
                        color = "green";
                        pageurl = "/Resumes/Resumes?Status=-1";
                    }
                    ...
                    {
                        color = "yellow";
                        pageurl = "/Resumes/Resumes?Status=2";
                    }
            ...
            strNotiHtml = strNotiHtml + "</ul></li><li class='footer'><a href='../Resumes/Resumes'>View all tasks</a></li></ul>";
            //strNotiHtml = "";
            ObjBlmenu = null;
